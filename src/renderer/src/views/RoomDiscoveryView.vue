  ...
  <div class="room-discovery-page">
    ...
    >
      ...
      <n-tab-pane name="lan" :tab="t('roomDiscovery.lanTab')">
        <n-spin
          :show="loading && activeTab === 'lan'"
          content-class="room-spin-content"
        >
...
const relayLatencyText = computed(
  () =>
    `${t("roomDiscovery.latency")}: ${relayLatencyMs.value === null ? "-- ms" : `${Math.round(relayLatencyMs.value)} ms`}`,
);
...
const refreshCurrentTab = () => {
  void refreshTab(activeTab.value, true);
};

const refreshTab = async (tab: "lan" | "relay", useCooldown: boolean) => {
  if (useCooldown && (!canRefresh.value || loading.value)) return;
  const requestId = ++refreshRequestId;
  loading.value = true;
  try {
    await doRefreshTab(tab);
  } catch (error: any) {
...
const doRefreshTab = async (tab: "lan" | "relay") => {
  if (tab === "lan") {
...
const joinBlockText = (room: DiscoveredRoom) => {
  if (room.joinBlockReason === "game_missing")
    return t("roomDiscovery.gameMissing");
  if (room.joinBlockReason === "version_mismatch")
    return t("roomDiscovery.versionMismatch");
  if (room.joinBlockReason === "room_full") return t("roomDiscovery.roomFull");
  if (room.joinBlockReason === "game_started")
    return t("roomDiscovery.gameStarted");
  if (room.joinBlockReason === "own_room") return t("roomDiscovery.ownRoom");
  return t("roomDiscovery.unavailable");
};

const roomGameName = (room: DiscoveredRoom) =>
  gameStore.games.find((game) => game.id === room.gameId)?.name ||
  room.gameName ||
  room.gameId;

const relayRoomAddress = (room: DiscoveredRoom) =>
  room.address || t("roomDiscovery.relayAddressPending");

const roomKey = (room: DiscoveredRoom) =>
  `${room.source}:${room.id}:${room.address}`;

const isJoiningRoom = (room: DiscoveredRoom) =>
...
const joinButtonType = (room: DiscoveredRoom) =>
  room.canJoin || room.joinBlockReason === "own_room" ? "primary" : "default";
...
const performDiscoveredRoomJoin = async (
  room: DiscoveredRoom,
  password?: string,
) => {
...
const startRelayLatencyRefresh = () => {
  ...
  relayLatencyTimer = window.setInterval(() => {
    void refreshRelayInBackground();
  }, RoomConstants.RELAY_LATENCY_REFRESH_INTERVAL_MS);
};

const refreshRelayInBackground = async () => {
  if (activeTab.value !== "relay" || loading.value || relayRefreshInFlight) {
    return;
  }
  relayRefreshInFlight = true;
  try {
    await doRefreshTab("relay");
  } catch (error) {
    console.warn("[RoomDiscovery] Background relay refresh failed:", error);
  } finally {
...
const startRefreshCooldown = () => {
  ...
  }, RoomConstants.ROOM_REFRESH_COOLDOWN_MS);
};

const stopRefreshCooldown = () => {
...
const RoomList = () => {
  ...
  return h(
    ...
    displayedRooms.value.map((room) =>
      h(
        NCard,
        { key: roomKey(room), class: "room-card" },
        {
          header: () =>
            h(
              NSpace,
            ...
            h(
              ...
              { column: 2, size: "small" },
              {
                default: () => [
                  h(
                    NDescriptionsItem,
                    { label: t("roomDiscovery.gameId") },
                    { default: () => room.gameId },
                  ),
                  h(
                    NDescriptionsItem,
                    { label: t("roomDiscovery.gameName") },
                    { default: () => roomGameName(room) },
                  ),
                  h(
                    NDescriptionsItem,
                    { label: t("roomDiscovery.gameVersion") },
                    { default: () => room.gameVersion },
                  ),
                  h(
                    NDescriptionsItem,
                    { label: t("roomDiscovery.players") },
                    { default: () => `${room.playerCount}/${room.maxPlayers}` },
                  ),
                  h(
                    NDescriptionsItem,
                    { label: t("roomDiscovery.address") },
                    {
                      default: () =>
                        room.source === "relay"
                          ? relayRoomAddress(room)
                          : room.address,
                    },
                  ),
                ],
