...
export const normalizeRoomJoinAddress = (address: string) => {
  ...
  const relayPattern = new RegExp(
    `^${escapeRegExp(DEFAULT_RELAY_PUBLIC_HOST)}:\\d+$`,
    "i",
  );
  if (relayPattern.test(trimmed) || /^(ws|wss):\/\//i.test(trimmed))
    return trimmed;
...
export const useRoomJoin = () => {
  ...
  const joinErrorText = (error?: string) => {
    if (error === "version_mismatch")
      return t("room.joinError.versionMismatch");
    ...
    if (error === "close_current_room_failed")
      return t("room.joinError.closeCurrentRoomFailed");
    if (error === "password_required")
      return t("room.joinError.passwordRequired");
    if (error === "password_incorrect")
      return t("room.joinError.passwordIncorrect");
    if (error === "manifestInvalid") return t("room.joinError.manifestInvalid");
    if (error === "gameTypeNotMultiplayer")
      return t("room.joinError.gameTypeNotMultiplayer");
    if (error === "multiplayerConfigMissing")
      return t("room.joinError.multiplayerConfigMissing");
    if (error === "platformVersionMismatch")
      return t("room.joinError.platformVersionMismatch");
  ...
  const joinRoomByAddress = async (options: JoinRoomOptions) => {
    ...
    try {
      const res = await roomStore.joinRoom(
        options.gameId,
        address,
        options.version,
  ...
  const saveLastJoinAddress = async (address: string) => {
    try {
      await settingsStore.savePartialSettings({ lastJoinRoomAddress: address });
    } catch {}
  };
