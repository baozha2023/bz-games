...
export class GameApiServer {
    ...
    private async handleAchievementList(ws: WebSocket, req: GameApiRequest) {
        ...
        const gameVersion = game?.versions.find(v => v.version === this.gameVersion);
    ...
    private async handleAchievementUnlock(ws: WebSocket, req: GameApiRequest) {
        ...
        const unlocked = storeService.unlockAchievement(this.gameId, this.gameVersion, achievementId);
        if (unlocked) {
