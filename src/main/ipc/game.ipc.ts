...
export function registerGameIpc() {
  ...
  ipcMain.handle(
    ...
    async (_, id: string, version: string, achievementId: string) => {
      try {
        const versionPath = await GameLoader.getVersionPath(id, version);
        const manifest = await GameLoader.getManifest(id, version);
        const relativePath = manifest?.achievements?.find(
          (achievement) => achievement.id === achievementId,
        )?.icon;
        if (!versionPath || !relativePath) return null;
        const absolutePath = path.join(versionPath, relativePath);
        if (
        ...
        return `data:image/${extension};base64,${fs.readFileSync(
          absolutePath,
          "base64",
        )}`;
      } catch (error) {
        logger.error(
          `[GameIPC] Failed to read achievement icon for ${id} @ ${version} / ${achievementId}`,
          error,
        );
        return null;
      }
