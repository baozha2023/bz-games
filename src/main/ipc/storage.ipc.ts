...
export function registerStorageIpc() {
  ...
  ipcMain.on(IPC.GAME_STORAGE_INIT, (event, gameId, version) => {
    try {
      ...
      console.error(`[Storage] Failed to load data for ${gameId} @ ${version}:`, error);
      event.returnValue = {};
    }
  });
...
function updateStorage(gameId: string, version: string, updateFn: (data: Record<string, any>) => void) {
  if (!gameId) return;
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.warn(`[Storage] Corrupt storage file for ${gameId} @ ${version}, resetting.`);
      }
    }
    ...
    console.error(`[Storage] Failed to save data for ${gameId} @ ${version}:`, error);
  }
}
