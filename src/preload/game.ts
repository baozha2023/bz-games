...
class GameStorage implements Storage {
  ...
  constructor(gameId: string, version: string, initialData: Record<string, string>) {
    this._gameId = gameId
  ...
  setItem(key: string, value: string): void {
    ...
    ipcRenderer.send('game:storage:save', this._gameId, this._version, key, stringValue)
  }
}
...
function init() {
  ...
  if (gameId) {
    console.log(`[GamePreload] Initializing storage for game: ${gameId} @ ${version}`)
    
    try {
      // 同步获取初始数据
      const initialData = ipcRenderer.sendSync('game:storage:init', gameId, version)
    ...
    console.warn('[GamePreload] No gameId found in URL, skipping storage override')
  }
}
