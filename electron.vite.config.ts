...
type PrivateBuildConfig = {
  ...
  previousReleaseFeedUrl?: string;
};

const PRIVATE_CONFIG_FILES = new Set([
  "private-build.config.json",
  "private-build.config.test.json",
]);
const privateConfigFile =
  process.env.BZ_PRIVATE_BUILD_CONFIG || "private-build.config.json";
if (!PRIVATE_CONFIG_FILES.has(privateConfigFile)) {
  throw new Error(
    `[BZ-Games] unsupported private build config: ${privateConfigFile}`,
  );
}
const privateConfigPath = resolve(privateConfigFile);
...
function requireEncryptionSeed(configKey: string, value?: string): string {
  ...
  if (decoded.length !== 32 || canonical !== seed) {
    throw new Error(
      `[BZ-Games] ${privateConfigFile} must define ${configKey} ` +
...
const injectedPrivateConfig = {
  ...
  __BZ_UPDATE_FEED_URL__: JSON.stringify(
    privateBuildConfig.updateFeedUrl || "",
  ),
  __BZ_TEST_CHANNEL__: JSON.stringify(
    privateConfigFile === "private-build.config.test.json",
  ),
};
    ...
    build: {
      rollupOptions: {
        input: {
          ...
          updateDownloadWorker: resolve("src/main/updateDownloadWorker.ts"),
        },
