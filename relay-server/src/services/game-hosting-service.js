...
function validateManifestCommon(raw) {
  ...
  if (value.author !== undefined)
    result.author = requiredString(
      value.author,
      1,
      100,
      "invalid_game_manifest",
    );
  if (value.author_url !== undefined)
    result.author_url = httpUrl(value.author_url, "invalid_game_manifest");
  if (value.platformVersion !== undefined)
    result.platformVersion = validatePlatformVersion(
  ...
  if (value.web_url !== undefined)
    result.web_url = httpUrl(value.web_url, "invalid_game_manifest");
  for (const key of ["icon", "cover", "video"]) {
    if (value[key] !== undefined)
      result[key] = safeManifestPath(value[key], "invalid_game_manifest");
  }
  if (value.encryptLocalStorage !== undefined)
    result.encryptLocalStorage = optionalBoolean(
      value.encryptLocalStorage,
      "invalid_game_manifest",
    );
  if (value.windowedFullscreen !== undefined)
    result.windowedFullscreen = optionalBoolean(
      value.windowedFullscreen,
      "invalid_game_manifest",
    );
  if (value.type !== undefined) {
    if (!GAME_TYPES.has(value.type))
      throw new GameHostingError("invalid_game_manifest");
    result.type = value.type;
  }
  if (value.multiplayer !== undefined) {
    const multiplayer = assertObject(
      value.multiplayer,
      "invalid_game_manifest",
    );
    assertKnownKeys(
      multiplayer,
      new Set(["minPlayers", "maxPlayers"]),
      "invalid_game_manifest",
    );
    const minPlayers = optionalPositiveInteger(
      multiplayer.minPlayers,
      "invalid_game_manifest",
    );
    const maxPlayers = optionalPositiveInteger(
  ...
  if (value.args !== undefined)
    result.args = validateStringArray(
      value.args,
      256,
      8192,
      "invalid_game_manifest",
  ...
  if (value.env !== undefined) {
    ...
    for (const [key, item] of Object.entries(env)) {
      if (
        ...
        key.toUpperCase().startsWith("BZ_") ||
  ...
  if (value.entry !== undefined) {
    ...
    const isWebEntry =
      ...
      entry.endsWith(".html") ||
      entry.endsWith(".htm");
    if (isWebEntry && value.args !== undefined && value.args.length > 0)
      throw new GameHostingError("invalid_game_manifest");
    if (
      isWebEntry &&
      value.env !== undefined &&
      ...
      throw new GameHostingError("invalid_game_manifest");
    if (!isWebEntry && value.windowedFullscreen !== undefined)
      throw new GameHostingError("invalid_game_manifest");
  }
  return result;
}

function validateManifestItemId(value) {
  const id = requiredString(value, 1, 200, "invalid_game_manifest");
  if (
...
function validateManifestV2(raw) {
  const value = assertObject(raw, "invalid_game_manifest");
  assertKnownKeys(
    value,
    new Set([
  ...
  if (!SUPPORTED_LOCALES.has(value.defaultLocale))
    throw new GameHostingError("invalid_game_manifest");
  if (
    !Array.isArray(statistics) ||
    !Array.isArray(achievements) ||
    !isPlainObject(localizations)
  )
    throw new GameHostingError("invalid_game_manifest");
  const result = {
    manifestVersion: 2,
    defaultLocale: value.defaultLocale,
    ...validateManifestCommon(common),
  };

  const statisticIds = new Set();
  if (statistics !== undefined) {
    if (!Array.isArray(statistics) || statistics.length > 1000)
      throw new GameHostingError("invalid_game_manifest");
    result.statistics = statistics.map((item) => {
      const definition = assertObject(item, "invalid_game_manifest");
      assertKnownKeys(
        definition,
        new Set(["id", "mode"]),
        "invalid_game_manifest",
      );
      const id = validateManifestItemId(definition.id);
      if (
        statisticIds.has(id) ||
        !["increment", "full"].includes(definition.mode)
      ) {
        throw new GameHostingError("invalid_game_manifest");
      }
      statisticIds.add(id);
      return { id, mode: definition.mode };
    });
  }

  const achievementIds = new Set();
  if (achievements !== undefined) {
    if (!Array.isArray(achievements) || achievements.length > 1000)
      throw new GameHostingError("invalid_game_manifest");
    result.achievements = achievements.map((item) => {
      const definition = assertObject(item, "invalid_game_manifest");
      assertKnownKeys(
        definition,
        new Set(["id", "icon"]),
      ...
      return {
        ...
        ...(definition.icon === undefined
          ? {}
          : {
              icon: safeManifestPath(definition.icon, "invalid_game_manifest"),
            }),
      };
    });
  }
  ...
  if (localizations !== undefined) {
    const bundles = assertObject(localizations, "invalid_game_manifest");
    result.localizations = {};
    for (const [locale, rawBundle] of Object.entries(bundles)) {
      if (!SUPPORTED_LOCALES.has(locale))
        throw new GameHostingError("invalid_game_manifest");
      const bundle = assertObject(rawBundle, "invalid_game_manifest");
      assertKnownKeys(
        bundle,
        new Set(["name", "description", "achievements", "statistics"]),
        "invalid_game_manifest",
      );
      const name = requiredString(bundle.name, 1, 100, "invalid_game_manifest");
      ...
      const localizedAchievements = assertObject(
        bundle.achievements,
        "invalid_game_manifest",
      );
      const localizedStatistics = assertObject(
        bundle.statistics,
        "invalid_game_manifest",
      );
      if (
        Object.keys(localizedAchievements).length !== achievementIds.size ||
        Object.keys(localizedStatistics).length !== statisticIds.size
      ) {
        throw new GameHostingError("invalid_game_manifest");
      }
      const normalizedAchievements = {};
      for (const [id, rawDefinition] of Object.entries(localizedAchievements)) {
        if (!achievementIds.has(id))
          throw new GameHostingError("invalid_game_manifest");
        const definition = assertObject(rawDefinition, "invalid_game_manifest");
        assertKnownKeys(
          definition,
          new Set(["title", "description"]),
          "invalid_game_manifest",
        );
        normalizedAchievements[id] = {
          ...
          description:
            ...
            definition.description.length <= 1000
              ? definition.description.normalize("NFC")
              : (() => {
                  throw new GameHostingError("invalid_game_manifest");
                })(),
        };
      ...
      for (const [id, label] of Object.entries(localizedStatistics)) {
        if (!statisticIds.has(id))
          throw new GameHostingError("invalid_game_manifest");
        normalizedStatistics[id] = requiredString(
          label,
          1,
          200,
          "invalid_game_manifest",
    ...
    if (!result.localizations[value.defaultLocale])
      throw new GameHostingError("invalid_game_manifest");
  }
  return result;
}

function validateLocalizationMap(raw, validateBundle, code) {
  const value = assertObject(raw, code);
  const entries = Object.entries(value);
  if (!entries.length) throw new GameHostingError(code);
  const result = {};
  for (const [locale, bundle] of entries) {
    if (!SUPPORTED_LOCALES.has(locale)) throw new GameHostingError(code);
    result[locale] = validateBundle(bundle);
  }
  return result;
}
...
function validateGameMetadata(raw, expectedId) {
  ...
  if (!GAME_ID_PATTERN.test(id) || (expectedId && id !== expectedId))
    throw new GameHostingError("invalid_game_id");
  if (!GAME_TYPES.has(value.type))
    throw new GameHostingError("invalid_game_metadata");
  if (!SUPPORTED_LOCALES.has(value.defaultLocale))
    throw new GameHostingError("invalid_game_metadata");
  const localizations = validateLocalizationMap(
    value.localizations,
    (rawBundle) => {
      const bundle = assertObject(rawBundle, "invalid_game_metadata");
      assertKnownKeys(
        bundle,
        new Set(["name", "summary", "tags"]),
        "invalid_game_metadata",
      );
      return {
        name: requiredString(bundle.name, 1, 100, "invalid_game_metadata"),
  ...
  if (!localizations[value.defaultLocale])
    throw new GameHostingError("invalid_game_metadata");
  const result = {
    id,
    defaultLocale: value.defaultLocale,
  ...
  if (value.author_url !== undefined)
    result.author_url = httpUrl(value.author_url, "invalid_game_metadata");
  if (value.iconUrl !== undefined)
    result.iconUrl = optionalHttpOrHostedUrl(
      value.iconUrl,
      "icon",
      "invalid_game_metadata",
    );
  if (value.coverUrl !== undefined)
    result.coverUrl = optionalHttpOrHostedUrl(
      value.coverUrl,
      "cover",
      "invalid_game_metadata",
    );
  if (value.screenshots !== undefined)
    result.screenshots = validateStringArray(
      value.screenshots,
      100,
      2048,
      "invalid_game_metadata",
    ).map((item) => httpUrl(item, "invalid_game_metadata"));
  if (value.featured !== undefined)
    result.featured = optionalBoolean(value.featured, "invalid_game_metadata");
  if (value.visibility !== undefined) {
    if (!VISIBILITIES.has(value.visibility))
      throw new GameHostingError("invalid_game_metadata");
    result.visibility = value.visibility;
  }
  if (value.minPlayers !== undefined)
    result.minPlayers = optionalPositiveInteger(
      value.minPlayers,
      "invalid_game_metadata",
    );
  if (value.maxPlayers !== undefined)
    result.maxPlayers = optionalPositiveInteger(
      value.maxPlayers,
      "invalid_game_metadata",
    );
  if (
...
function validateVersionMetadata(raw) {
  ...
  if (!SEMVER_PATTERN.test(version))
    throw new GameHostingError("invalid_game_version");
  const result = {
    version,
    localizations: validateLocalizationMap(
      value.localizations,
      (rawBundle) => {
        const bundle = assertObject(rawBundle, "invalid_game_version_metadata");
        assertKnownKeys(
          bundle,
          new Set(["description", "releaseNotes"]),
          "invalid_game_version_metadata",
        );
        const localization = {
        ...
        if (bundle.releaseNotes !== undefined) {
          if (typeof bundle.releaseNotes !== "string")
            throw new GameHostingError("invalid_game_version_metadata");
          if (!bundle.releaseNotes.trim()) return localization;
          localization.releaseNotes = optionalString(
            bundle.releaseNotes,
            20000,
            "invalid_game_version_metadata",
          );
    ...
    platformVersion: validatePlatformVersion(
      value.platformVersion,
      "invalid_game_version_metadata",
    ),
  };
  ...
  if (value.isPrerelease !== undefined)
    result.isPrerelease = optionalBoolean(
      value.isPrerelease,
      "invalid_game_version_metadata",
    );
  if (value.gameManifest !== undefined)
    result.gameManifest = validateManifestV2(value.gameManifest);
  return result;
}

function assertVersionMatchesGame(game, version) {
  const gameLocales = Object.keys(game.localizations).sort();
  const versionLocales = Object.keys(version.localizations).sort();
  if (
    gameLocales.length !== versionLocales.length ||
    gameLocales.some((locale, index) => locale !== versionLocales[index])
  ) {
    throw new GameHostingError("invalid_game_version_metadata");
  }
  const manifestLocales = Object.keys(
    version.gameManifest?.localizations || {},
  ).sort();
  if (
    version.gameManifest &&
    version.gameManifest.defaultLocale !== game.defaultLocale
  )
    throw new GameHostingError("invalid_game_manifest");
  if (
    manifestLocales.length > 0 &&
    (manifestLocales.length !== gameLocales.length ||
      manifestLocales.some((locale, index) => locale !== gameLocales[index]))
  ) {
    throw new GameHostingError("invalid_game_manifest");
  }
  if (!version.gameManifest) return;
  const manifest = version.gameManifest;
  ...
  const isWebEntry =
    normalizedEntry === "url" ||
    normalizedEntry === "serve" ||
    normalizedEntry.endsWith(".html") ||
    normalizedEntry.endsWith(".htm");
  if (
    (normalizedEntry === "url" && !manifest.web_url) ||
    (normalizedEntry !== "url" && manifest.web_url)
  ) {
    throw new GameHostingError("invalid_game_manifest");
  }
  if (
    (isWebEntry && Array.isArray(manifest.args) && manifest.args.length > 0) ||
    (isWebEntry && manifest.env && Object.keys(manifest.env).length > 0) ||
    (!isWebEntry && manifest.windowedFullscreen !== undefined) ||
    (!isWebEntry && manifest.encryptLocalStorage === true)
  ) {
    throw new GameHostingError("invalid_game_manifest");
  }

  const type = manifest.type || game.type;
  if (type === "networkgame" && normalizedEntry !== "url") {
    throw new GameHostingError("invalid_game_manifest");
  }
  const multiplayerType = type === "multiplayer" || type === "singlemultiple";
  const inheritedMultiplayer =
    game.minPlayers != null
      ...
      : undefined;
  const multiplayer = manifest.multiplayer || inheritedMultiplayer;
  if ((multiplayerType && !multiplayer) || (!multiplayerType && multiplayer)) {
    throw new GameHostingError("invalid_game_manifest");
  }
}
...
function detectAssetType(role, header) {
  if (role === "package") {
    if (!ZIP_SIGNATURES.has(header.subarray(0, 4).toString("hex")))
      throw new GameHostingError("invalid_zip_archive");
...
export function parseLogicalUrl(value) {
  if (
    typeof value !== "string" ||
    !value.startsWith(HOSTED_GAME_LOGICAL_PREFIX)
  )
    return null;
...
async function parseMultipartUpload(
  ...
  if (!contentType.toLowerCase().startsWith("multipart/form-data"))
    throw new GameHostingError("multipart_required", 415);
  const maxBody = limits.package + limits.image * 2 + 2 * 1024 * 1024;
  const declaredLength = Number(req.headers["content-length"] || 0);
  if (Number.isFinite(declaredLength) && declaredLength > maxBody)
    throw new GameHostingError("request_too_large", 413);
  ...
  const fail = (error) => {
    parseError ||=
      error instanceof GameHostingError
        ? error
        : new GameHostingError("invalid_multipart");
  };
  busboy.on("field", (name, value, info) => {
    if (
      info.nameTruncated ||
      info.valueTruncated ||
      !["game", "version", "setLatest"].includes(name) ||
  ...
  busboy.on("file", (role, stream, info) => {
    ...
    stream.once("limit", () =>
      fail(
        new GameHostingError(
          role === "package"
            ? "game_archive_too_large"
      ...
      transform(chunk, _encoding, callback) {
        ...
        if (header.length < 16)
          header = Buffer.concat([
            header,
            chunk.subarray(0, 16 - header.length),
          ]);
        hash.update(chunk);
        callback(null, chunk);
      },
  ...
  busboy.on("partsLimit", () =>
    fail(new GameHostingError("invalid_multipart")),
  );
  ...
  if (requirePackage && !assets.has("package"))
    throw new GameHostingError("game_archive_required");
  if (typeof fields.version !== "string")
    throw new GameHostingError("invalid_game_version_metadata");
  if (requireGame && typeof fields.game !== "string")
    throw new GameHostingError("invalid_game_metadata");
  const game = requireGame
    ? validateGameMetadata(parseJson(fields.game, "invalid_game_metadata"))
    : null;
  const version = validateVersionMetadata(
    parseJson(fields.version, "invalid_game_version_metadata"),
  );
  const setLatest =
    fields.setLatest === undefined ? true : fields.setLatest === "true";
  if (
    fields.setLatest !== undefined &&
    !["true", "false"].includes(fields.setLatest)
  )
    throw new GameHostingError("invalid_upload_fields");
  if (assets.has("icon") && game?.iconUrl)
    throw new GameHostingError("duplicate_icon_source");
  if (assets.has("cover") && game?.coverUrl)
    throw new GameHostingError("duplicate_cover_source");
...
function compareSemverDescending(left, right) {
  ...
  for (
    ...
    if (an && bn && Number(a.pre[index]) !== Number(b.pre[index]))
      return Number(b.pre[index]) - Number(a.pre[index]);
...
export function createGameHostingService({
  config,
  mySqlService,
  accessControlService,
}) {
  const canManageAll = (auth) =>
    auth.can(PORTAL_CAPABILITIES.HOSTING_ALL_MANAGE);
  const canPublishDirectly = (auth) =>
    auth.can(PORTAL_CAPABILITIES.HOSTING_PUBLISH_DIRECT);
  ...
  if (
    !Number.isSafeInteger(config.MAX_GAME_HOSTING_FILE_BYTES) ||
    config.MAX_GAME_HOSTING_FILE_BYTES <= 0 ||
    !Number.isSafeInteger(config.MAX_GAME_HOSTING_IMAGE_BYTES) ||
    config.MAX_GAME_HOSTING_IMAGE_BYTES <= 0 ||
    config.MAX_GAME_HOSTING_IMAGE_BYTES > config.MAX_GAME_HOSTING_FILE_BYTES ||
    !Number.isSafeInteger(config.MAX_GAME_HOSTING_TOTAL_BYTES) ||
    config.MAX_GAME_HOSTING_TOTAL_BYTES < config.MAX_GAME_HOSTING_FILE_BYTES
  )
  ...
  function versionDirectory(gameId, version) {
    if (!GAME_ID_PATTERN.test(gameId) || !SEMVER_PATTERN.test(version))
      throw new GameHostingError("invalid_storage_path", 500);
  ...
  function assertCreatorImageReferences(metadata, publishedMetadata = null) {
    for (const role of ["icon", "cover"]) {
      ...
      if (value !== publishedMetadata?.[`${role}Url`])
        throw new GameHostingError("invalid_game_metadata");
    }
  }
  ...
  async function installVersionFiles(parsed, gameId, version, finalDirectory) {
    for (const asset of parsed.assets) {
      await fs.rename(
        asset.tempPath,
        path.join(path.dirname(asset.tempPath), asset.storageName),
      );
      asset.tempPath = path.join(
        path.dirname(asset.tempPath),
        asset.storageName,
      );
    }
    await fs.mkdir(path.dirname(finalDirectory), {
      recursive: true,
      mode: 0o750,
    });
  ...
  async function createVersion(req, res, gameIdFromRoute) {
    ...
    try {
      ...
      if (
        gameIdFromRoute &&
        (!GAME_ID_PATTERN.test(gameIdFromRoute) || parsed.game)
      )
        throw new GameHostingError("invalid_game_id");
      finalDirectory = versionDirectory(gameId, parsed.version.version);
      const totalBytes = parsed.assets.reduce(
        (sum, asset) => sum + asset.size,
        0,
      );
      await withStorageLock(async () => {
        const [gameRows] = await mySqlService.query(
          "SELECT game_id, published_metadata_json, latest_version, owner_user_id FROM hosted_games WHERE game_id = ? LIMIT 1",
          [gameId],
        );
        if (!gameIdFromRoute && gameRows[0])
          throw new GameHostingError("hosted_game_exists", 409);
        if (gameIdFromRoute && !gameRows[0])
          throw new GameHostingError("hosted_game_not_found", 404);
        if (
          gameIdFromRoute &&
          !canManageOwnedResource(auth, gameRows[0].owner_user_id)
        )
          throw new GameHostingError("forbidden", 403);
        const [versionRows] = await mySqlService.query(
          "SELECT id FROM hosted_game_versions WHERE game_id = ? AND version = ? LIMIT 1",
          [gameId, parsed.version.version],
        );
        if (versionRows[0])
          throw new GameHostingError("hosted_game_version_exists", 409);
        ...
        if (
          ...
          parsed.assets.some(
            (asset) => asset.role === "icon" || asset.role === "cover",
          )
        ) {
          throw new GameHostingError("hosted_version_images_require_unique");
        }
        if (
          (await directoryUsage(filesDir)) + totalBytes >
          config.MAX_GAME_HOSTING_TOTAL_BYTES
        )
          throw new GameHostingError("game_hosting_capacity_exceeded", 507);
        try {
        ...
        let gameMetadata =
          parsed.game ||
          (gameRows[0].published_metadata_json
            ? parseDatabaseJson(gameRows[0].published_metadata_json)
            : null);
        if (!gameMetadata)
          throw new GameHostingError("game_metadata_not_approved", 409);
        assertVersionMatchesGame(gameMetadata, parsed.version);
        if (!publishDirectly && parsed.game)
          assertCreatorImageReferences(parsed.game);
        if (publishDirectly && gameMetadata)
          for (const asset of parsed.assets) {
            if (asset.role === "icon")
              gameMetadata = {
                ...gameMetadata,
                iconUrl: toLogicalUrl(
                  gameId,
        ...
        try {
          await mySqlService.transaction(async (connection) => {
            ...
            if (!gameIdFromRoute) {
              await connection.query(
                ...
                [
                  gameId,
                  publishDirectly ? JSON.stringify(gameMetadata) : null,
                  publishDirectly ? parsed.version.version : null,
                  auth.user.id,
                  auth.user.login || "",
                  auth.user.id,
                  auth.user.login || "",
              ...
              const latestVersion = parsed.setLatest
                ? parsed.version.version
                : gameRows[0].latest_version;
              await connection.query(
            ...
            await connection.query(
              ...
              [
                ...
                publishDirectly ? "approved" : "pending",
                initialRevisionId,
                auth.user.id,
                auth.user.login || "",
                publishDirectly ? auth.user.id : null,
                publishDirectly ? auth.user.login || "" : "",
                publishDirectly ? now : null,
                now,
                now,
              ],
            );
          ...
          try {
            await fs.rmdir(path.dirname(finalDirectory));
          } catch {}
  ...
  async function replaceVersion(req, res, gameId, version) {
    ...
    const publishDirectly = canPublishDirectly(auth);
    if (!GAME_ID_PATTERN.test(gameId) || !SEMVER_PATTERN.test(version))
      throw new GameHostingError("hosted_game_version_not_found", 404);
    await ensureStorage();
    ...
    try {
      ...
      await withStorageLock(async () => {
        ...
        if (!existing)
          throw new GameHostingError("hosted_game_version_not_found", 404);
        if (!canManageOwnedResource(auth, existing.owner_user_id))
          throw new GameHostingError("forbidden", 403);
        if (!existing.published_metadata_json)
          throw new GameHostingError("game_metadata_not_approved", 409);
        assertVersionMatchesGame(
          parseDatabaseJson(existing.published_metadata_json),
          parsed.version,
        );
        if (!canManageAll(auth)) {
          if (!["pending", "rejected"].includes(existing.status))
            throw new GameHostingError("approved_submission_read_only", 409);
        }
        const [versionCountRows] = await mySqlService.query(
          "SELECT COUNT(*) AS total FROM hosted_game_versions WHERE game_id = ?",
          [gameId],
        ...
        const [oldAssets] = await mySqlService.query(
          "SELECT role, size FROM hosted_game_assets WHERE version_id = ?",
          [existing.id],
        );
        const replacedRoles = new Set(parsed.assets.map((asset) => asset.role));
        const removedBytes = oldAssets
          .filter((asset) => replacedRoles.has(asset.role))
          .reduce((sum, asset) => sum + Number(asset.size), 0);
        const addedBytes = parsed.assets.reduce(
          (sum, asset) => sum + asset.size,
          0,
        );
        if (
          (await directoryUsage(filesDir)) - removedBytes + addedBytes >
          config.MAX_GAME_HOSTING_TOTAL_BYTES
        )
          throw new GameHostingError("game_hosting_capacity_exceeded", 507);
        ...
        moved = true;
        await fs.cp(quarantine, finalDirectory, {
          recursive: true,
          force: false,
        });
        for (const asset of parsed.assets) {
          for (const fileName of await fs.readdir(finalDirectory)) {
            if (fileName.startsWith(`${asset.role}.`))
              await fs.rm(path.join(finalDirectory, fileName), { force: true });
          }
          ...
          await fs.rename(
            stagedName,
            path.join(finalDirectory, asset.storageName),
          );
        }
        const now = new Date();
        try {
          await mySqlService.transaction(async (connection) => {
            await connection.query(
              ...
              [
                JSON.stringify(parsed.version),
                publishDirectly ? existing.status : "pending",
                now,
                existing.id,
              ],
            );
            if (
              !publishDirectly &&
              existing.initial_revision_id &&
              !existing.published_metadata_json
            ) {
              await connection.query(
            ...
            for (const asset of parsed.assets) {
              await connection.query(
                "DELETE FROM hosted_game_assets WHERE version_id = ? AND role = ?",
                [existing.id, asset.role],
              );
              await connection.query(
                `INSERT INTO hosted_game_assets
                  (id, version_id, role, original_name, storage_name, content_type, size, sha256, created_at)
            ...
            if (
              ...
              const metadata = parseDatabaseJson(
                existing.published_metadata_json,
              );
              for (const asset of parsed.assets)
                if (asset.role === "icon" || asset.role === "cover") {
                  metadata[`${asset.role}Url`] = toLogicalUrl(
                    gameId,
                    version,
                    asset.role,
                    asset.originalName,
      ...
      sendJson(res, 200, {
        ok: true,
        status: publishDirectly ? undefined : "pending",
      });
    } finally {
      if (moved) {
        await fs
  ...
  async function buildGameConfig(gameId) {
    const [games] = await mySqlService.query(
      "SELECT game_id, published_metadata_json, latest_version FROM hosted_games WHERE game_id = ? LIMIT 1",
      [gameId],
    );
    if (!games[0]) throw new GameHostingError("hosted_game_not_found", 404);
    if (!games[0].published_metadata_json)
      throw new GameHostingError("hosted_game_has_no_approved_versions", 409);
    const [rows] = await mySqlService.query(
    ...
    for (const row of rows) {
      if (!versions.has(row.id))
        versions.set(row.id, {
          metadata: parseDatabaseJson(row.metadata_json),
          assets: [],
        });
    ...
    for (const { metadata, assets } of versions.values()) {
      ...
      exportedVersions.push({
        ...metadata,
        downloadUrl: toLogicalUrl(
          gameId,
          metadata.version,
          "package",
          packageAsset.original_name,
        ),
        sha256: packageAsset.sha256,
        size: Number(packageAsset.size),
      });
    }
    exportedVersions.sort((a, b) =>
      compareSemverDescending(a.version, b.version),
    );
    if (!exportedVersions.length)
      throw new GameHostingError("hosted_game_has_no_approved_versions", 409);
    const configuredLatest = exportedVersions.some(
      (item) => item.version === games[0].latest_version,
    )
      ? games[0].latest_version
      : exportedVersions[0].version;
    return {
      ...parseDatabaseJson(games[0].published_metadata_json),
      latestVersion: configuredLatest,
  ...
  async function handleTree(req, res, url) {
    ...
    if (
      ...
      throw new GameHostingError("invalid_pagination");
    ...
    const params = [];
    if (!canManageAll(auth)) {
      clauses.push("g.owner_user_id = ?");
      params.push(auth.user.id);
    }
    if (query) {
      clauses.push(`(g.game_id LIKE ? ESCAPE '\\\\' OR g.owner_github_login LIKE ? ESCAPE '\\\\'
        OR COALESCE(JSON_UNQUOTE(JSON_EXTRACT(g.published_metadata_json, '$.localizations.\"zh-CN\".name')), '') LIKE ? ESCAPE '\\\\'
        OR EXISTS (SELECT 1 FROM hosted_game_metadata_revisions sr WHERE sr.game_id = g.game_id
                   AND JSON_UNQUOTE(JSON_EXTRACT(sr.metadata_json, '$.localizations.\"zh-CN\".name')) LIKE ? ESCAPE '\\\\')
    ...
    const [counts] = await mySqlService.query(
      `SELECT COUNT(*) AS total FROM hosted_games g ${where}`,
      params,
    );
    const [games] = await mySqlService.query(
    ...
    for (const row of rows) {
      ...
      if (!version) {
        version = {
          ...
          metadata: parseDatabaseJson(row.metadata_json),
          status: row.status,
          initialRevisionId: row.initial_revision_id || null,
          reviewReason: row.review_reason || "",
          uploader: row.uploader_github_login || "",
          reviewer: row.reviewer_github_login || "",
    ...
    for (const versions of byGame.values())
      versions.sort((a, b) => compareSemverDescending(a.version, b.version));
    let revisions = [];
    if (games.length) {
      const placeholders = games.map(() => "?").join(",");
    ...
    for (const revision of revisions)
      revisionsByGame.get(revision.game_id).push({
        id: revision.id,
        metadata: parseDatabaseJson(revision.metadata_json),
        status: revision.status,
        reviewReason: revision.review_reason || "",
        submitter: revision.submitter_github_login || "",
        reviewer: revision.reviewer_github_login || "",
    ...
    const initialRevisionIds = new Set(
      [...byVersion.values()]
        .map((version) => version.initialRevisionId)
        .filter(Boolean),
    );
    await ensureStorage();
    ...
    sendJson(res, 200, {
      games: games.map((game) => ({
        gameId: game.game_id,
        metadata: game.published_metadata_json
          ? parseDatabaseJson(game.published_metadata_json)
          : revisionsByGame.get(game.game_id)?.[0]?.metadata || {
              id: game.game_id,
              defaultLocale: "zh-CN",
              localizations: {
                "zh-CN": {
        ...
        published: Boolean(game.published_metadata_json),
        latestVersion: game.latest_version,
        owner: game.owner_github_login || "",
        updater: game.updated_by_github_login || "",
        createdAt: game.created_at,
        updatedAt: game.updated_at,
        revisions: revisionsByGame
          .get(game.game_id)
  ...
  async function updateGame(req, res, gameId) {
    ...
    await mySqlService.transaction(async (connection) => {
      ...
      if (!games[0]) throw new GameHostingError("hosted_game_not_found", 404);
      if (!canManageOwnedResource(auth, games[0].owner_user_id))
        throw new GameHostingError("forbidden", 403);
      ...
      if (String(games[0].owner_user_id) !== String(auth.user.id))
        throw new GameHostingError("forbidden", 403);
      assertCreatorImageReferences(
        metadata,
        games[0].published_metadata_json
          ? parseDatabaseJson(games[0].published_metadata_json)
          : null,
      );
    ...
    sendJson(res, 200, {
      ok: true,
      status: canPublishDirectly(auth) ? "approved" : "pending",
      revisionId,
    });
  }
  ...
  async function updateVersion(req, res, gameId, version) {
    ...
    await mySqlService.transaction(async (connection) => {
      ...
      if (!rows[0])
        throw new GameHostingError("hosted_game_version_not_found", 404);
      if (!canManageOwnedResource(auth, rows[0].owner_user_id))
        throw new GameHostingError("forbidden", 403);
      if (!rows[0].published_metadata_json)
        throw new GameHostingError("game_metadata_not_approved", 409);
      assertVersionMatchesGame(
        parseDatabaseJson(rows[0].published_metadata_json),
        metadata,
      );
      if (!canManageAll(auth)) {
      ...
      const [result] = await connection.query(
        ...
        [
          JSON.stringify(metadata),
          canPublishDirectly(auth) ? rows[0].status : "pending",
      ...
      if (!result.affectedRows)
        throw new GameHostingError("hosted_game_version_not_found", 404);
      if (
        !canPublishDirectly(auth) &&
        rows[0].initial_revision_id &&
        !rows[0].published_metadata_json
      ) {
        await connection.query(
          `UPDATE hosted_game_metadata_revisions SET status = 'pending', review_reason = '',
             reviewer_user_id = NULL, reviewer_github_login = '', reviewed_at = NULL, updated_at = ?
    ...
    sendJson(res, 200, {
      ok: true,
      status: canPublishDirectly(auth) ? undefined : "pending",
    });
  }
  ...
  async function setLatestVersion(req, res, gameId) {
    ...
    if (!SEMVER_PATTERN.test(version))
      throw new GameHostingError("invalid_game_version");
    const [rows] = await mySqlService.query(
      "SELECT id FROM hosted_game_versions WHERE game_id = ? AND version = ? AND status = 'approved' LIMIT 1",
      [gameId, version],
    );
    if (!rows[0])
      throw new GameHostingError("hosted_game_version_not_found", 404);
  ...
  async function deleteVersion(req, res, gameId, version) {
    ...
    await withStorageLock(async () => {
      ...
      const [gameRows] = await mySqlService.query(
        "SELECT published_metadata_json, latest_version FROM hosted_games WHERE game_id = ? LIMIT 1",
        [gameId],
      );
      if (!gameRows[0])
        throw new GameHostingError("hosted_game_not_found", 404);
      await quarantineAndDelete(versionDirectory(gameId, version), async () => {
        await mySqlService.transaction(async (connection) => {
          await connection.query(
            "DELETE FROM hosted_game_versions WHERE id = ?",
            [rows[0].id],
          );
          if (rows[0].initial_revision_id)
            await connection.query(
              "DELETE FROM hosted_game_metadata_revisions WHERE id = ?",
              [rows[0].initial_revision_id],
            );
          const [allRemaining] = await connection.query(
            "SELECT version, status FROM hosted_game_versions WHERE game_id = ?",
            [gameId],
          );
          const [revisionCount] = await connection.query(
            "SELECT COUNT(*) AS total FROM hosted_game_metadata_revisions WHERE game_id = ?",
            [gameId],
          );
          if (
            !allRemaining.length &&
            Number(revisionCount[0]?.total || 0) === 0
          )
            await connection.query(
              "DELETE FROM hosted_games WHERE game_id = ?",
              [gameId],
            );
          else {
            const approved = allRemaining
              .filter((item) => item.status === "approved")
              .sort((a, b) => compareSemverDescending(a.version, b.version));
            const currentLatestStillExists = approved.some(
              (item) => item.version === gameRows[0].latest_version,
            );
            const latestVersion = currentLatestStillExists
              ? gameRows[0].latest_version
              : approved[0]?.version || null;
            const metadata = gameRows[0].published_metadata_json
              ? parseDatabaseJson(gameRows[0].published_metadata_json)
              : null;
            if (metadata) {
              for (const role of ["icon", "cover"]) {
      ...
      try {
        await fs.rmdir(path.join(filesDir, gameId));
      } catch {}
    });
    sendJson(res, 200, { ok: true });
  }
  ...
  async function deleteGame(req, res, gameId) {
    ...
    await withStorageLock(async () => {
      const [rows] = await mySqlService.query(
        "SELECT game_id FROM hosted_games WHERE game_id = ? LIMIT 1",
        [gameId],
      );
  ...
  function parseReviewBody(body, allowSetLatest) {
    ...
    if (!["approved", "rejected"].includes(value.decision))
      throw new GameHostingError("invalid_review");
    ...
    const reason = rawReason.trim()
      ? optionalString(rawReason, 2000, "invalid_review")
      : "";
    if (value.decision === "rejected" && !reason)
      throw new GameHostingError("review_reason_required");
    if (value.setLatest !== undefined && typeof value.setLatest !== "boolean")
      throw new GameHostingError("invalid_review");
    return {
      decision: value.decision,
      reason,
      setLatest: Boolean(value.setLatest),
      expectedUpdatedAt,
    };
  }
  ...
  function assertUnchanged(actual, expected) {
    if (new Date(actual).toISOString() !== new Date(expected).toISOString())
      throw new GameHostingError("submission_changed", 409);
  }
  ...
  async function reviewVersion(req, res, versionId) {
    ...
    await mySqlService.transaction(async (connection) => {
      ...
      if (version.status !== "pending")
        throw new GameHostingError("submission_not_pending", 409);
      ...
      let metadata = version.published_metadata_json
        ? parseDatabaseJson(version.published_metadata_json)
        : null;
      if (version.initial_revision_id) {
        ...
        if (!revisions[0] || revisions[0].status !== "pending")
          throw new GameHostingError("submission_not_pending", 409);
      ...
      const [assets] = await connection.query(
        "SELECT role, original_name FROM hosted_game_assets WHERE version_id = ?",
        [versionId],
      );
      ...
      const [approvedRows] = await connection.query(
        "SELECT COUNT(*) AS total FROM hosted_game_versions WHERE game_id = ? AND status = 'approved'",
        [version.game_id],
      );
      const firstApproved = Number(approvedRows[0]?.total || 0) === 0;
      const latestVersion =
        firstApproved || review.setLatest
          ? version.version
    ...
    await mySqlService.transaction(async (connection) => {
      const [rows] = await connection.query(
        `SELECT r.*, g.published_metadata_json FROM hosted_game_metadata_revisions r
         JOIN hosted_games g ON g.game_id = r.game_id WHERE r.id = ? FOR UPDATE`,
        [revisionId],
      );
      const revision = rows[0];
      if (!revision)
        throw new GameHostingError("hosted_game_revision_not_found", 404);
      if (revision.status !== "pending")
        throw new GameHostingError("submission_not_pending", 409);
      assertUnchanged(revision.updated_at, review.expectedUpdatedAt);
      const [linked] = await connection.query(
        "SELECT id FROM hosted_game_versions WHERE initial_revision_id = ? LIMIT 1",
        [revisionId],
      );
      if (linked[0])
        throw new GameHostingError("initial_revision_review_with_version", 409);
      const now = new Date();
      await connection.query(
        ...
        [
          review.decision,
          review.decision === "rejected" ? review.reason : "",
          auth.user.id,
          auth.user.login || "",
          now,
          now,
      ...
      if (review.decision === "approved")
        await connection.query(
          `UPDATE hosted_games SET published_metadata_json = ?, updated_by_user_id = ?,
           updated_by_github_login = ?, updated_at = ? WHERE game_id = ?`,
          [
            JSON.stringify(parseDatabaseJson(revision.metadata_json)),
            auth.user.id,
            auth.user.login || "",
            now,
            revision.game_id,
  ...
  async function deleteRevision(req, res, revisionId) {
    ...
    await mySqlService.transaction(async (connection) => {
      ...
      if (linked[0])
        throw new GameHostingError("delete_initial_version_instead", 409);
      await connection.query(
        "DELETE FROM hosted_game_metadata_revisions WHERE id = ?",
        [revisionId],
      );
  ...
  async function handleDownload(
    ...
    if (
      ...
      throw new GameHostingError("hosted_game_asset_not_found", 404);
    const [rows] = await mySqlService.query(
    ...
    const asset = rows[0];
    if (!asset || asset.original_name !== fileName)
      throw new GameHostingError("hosted_game_asset_not_found", 404);
    const filePath = path.join(
      versionDirectory(gameId, version),
      asset.storage_name,
    );
    let stat;
    try {
    ...
    if (
      req.headers["if-none-match"] === `"${asset.sha256}"` &&
      !req.headers.range
    ) {
    ...
    const asciiName =
      role === "package"
        ? "game.zip"
        : `${role}.${asset.content_type.split("/")[1].replace("jpeg", "jpg")}`;
    const headers = {
  ...
  async function handlePortalDownload(req, res, rawAssetId) {
    ...
    if (!canManageOwnedResource(auth, asset.owner_user_id))
      throw new GameHostingError("forbidden", 403);
    ...
    const filePath = path.join(
      versionDirectory(asset.game_id, asset.version),
      storageName,
    );
    let stat;
    try {
    ...
    const end = range?.end ?? size - 1;
    const extension =
      String(asset.content_type || "application/octet-stream")
        .split("/")[1]
        ?.replace("jpeg", "jpg") || "bin";
    const asciiName =
      asset.role === "package" ? "game.zip" : `${asset.role}.${extension}`;
    const headers = {
      "content-type": asset.content_type,
      "content-length": String(end - start + 1),
  ...
  async function handleRequest(req, res, url) {
    const assetMatch = url.pathname.match(
      /^\/api\/v1\/game-hosting\/assets\/([^/]+)\/([^/]+)\/(package|icon|cover)\/([^/]+)$/,
    );
    const portalAssetDownloadMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/assets\/([^/]+)\/download$/,
    );
    const base = "/api/portal/v1/game-hosting";
    const versionMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/games\/([^/]+)\/versions\/([^/]+)$/,
    );
    const versionsCollectionMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/games\/([^/]+)\/versions$/,
    );
    const latestMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/games\/([^/]+)\/latest$/,
    );
    const configMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/games\/([^/]+)\/config$/,
    );
    const gameMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/games\/([^/]+)$/,
    );
    const reviewVersionMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/reviews\/versions\/([^/]+)$/,
    );
    const reviewRevisionMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/reviews\/revisions\/([^/]+)$/,
    );
    const revisionMatch = url.pathname.match(
      /^\/api\/portal\/v1\/game-hosting\/revisions\/([^/]+)$/,
    );
    const gamesCollection = url.pathname === `${base}/games`;
    const treeCollection = url.pathname === `${base}/tree`;
    if (
      !assetMatch &&
      !portalAssetDownloadMatch &&
      !versionMatch &&
      !versionsCollectionMatch &&
      !latestMatch &&
      !configMatch &&
      !gameMatch &&
      !reviewVersionMatch &&
    ...
    if (!mySqlService.isEnabled()) {
      sendJson(res, 503, { error: "game_hosting_storage_not_configured" });
      return true;
    }
    try {
      if (["GET", "HEAD"].includes(req.method) && assetMatch)
        await handleDownload(req, res, url, ...assetMatch.slice(1));
      else if (["GET", "HEAD"].includes(req.method) && portalAssetDownloadMatch)
        await handlePortalDownload(
          req,
          res,
          ...portalAssetDownloadMatch.slice(1),
        );
      else if (req.method === "POST" && gamesCollection)
        await createVersion(req, res, null);
      else if (req.method === "POST" && versionsCollectionMatch)
        await createVersion(
          req,
          res,
          decodeCanonicalSegment(versionsCollectionMatch[1]),
        );
      else if (req.method === "GET" && treeCollection)
        await handleTree(req, res, url);
        ...
        const [games] = await mySqlService.query(
          "SELECT owner_user_id FROM hosted_games WHERE game_id = ? LIMIT 1",
          [gameId],
        );
        if (!games[0]) throw new GameHostingError("hosted_game_not_found", 404);
        if (!canManageOwnedResource(auth, games[0].owner_user_id))
      ...
      } else if (req.method === "PUT" && gameMatch)
        await updateGame(req, res, decodeCanonicalSegment(gameMatch[1]));
        ...
        if (
          ...
          await replaceVersion(req, res, gameId, version);
        else await updateVersion(req, res, gameId, version);
      } else if (req.method === "PUT" && latestMatch)
        await setLatestVersion(
          req,
          res,
          decodeCanonicalSegment(latestMatch[1]),
        );
      else if (req.method === "PUT" && reviewVersionMatch)
        await reviewVersion(
          req,
          res,
          decodeCanonicalSegment(reviewVersionMatch[1]),
        );
      else if (req.method === "PUT" && reviewRevisionMatch)
        await reviewRevision(
          req,
          res,
          decodeCanonicalSegment(reviewRevisionMatch[1]),
        );
      else if (req.method === "DELETE" && revisionMatch)
        await deleteRevision(
          req,
          res,
          decodeCanonicalSegment(revisionMatch[1]),
        );
      else if (req.method === "DELETE" && versionMatch)
        await deleteVersion(
          req,
          res,
          decodeCanonicalSegment(versionMatch[1]),
          decodeCanonicalSegment(versionMatch[2]),
        );
      else if (req.method === "DELETE" && gameMatch)
        await deleteGame(req, res, decodeCanonicalSegment(gameMatch[1]));
      else sendJson(res, 404, { error: "not_found" });
    } catch (error) {
      const hostingError =
        error instanceof GameHostingError
          ? error
          : new GameHostingError("game_hosting_internal_error", 500);
      if (!(error instanceof GameHostingError))
        console.error("[relay-server] game hosting request failed", error);
      if (!res.headersSent)
        sendJson(res, hostingError.status, { error: hostingError.code });
      else res.destroy();
    }
    return true;
  }
