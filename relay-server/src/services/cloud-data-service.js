...
export function createCloudDataService({ config, authService, mongoService, mySqlService }) {
  ...
  async function getFileRef(userId, fileKey) {
    ...
    const [rows] = await mySqlService.query(
      "SELECT * FROM user_file_refs WHERE user_id = ? AND file_key = ? LIMIT 1",
      [userId, fileKey],
    );
    return rows[0] || null;
  }
  ...
  async function tryConsumeRateLimit(userId, actionType, operationId) {
    ...
    const limit = await mySqlService.transaction(async (connection) => {
      ...
      `SELECT last_action_at, operation_id
      ...
      `INSERT INTO cloud_sync_limits (user_id, action_type, operation_id, last_action_at)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE operation_id = VALUES(operation_id), last_action_at = VALUES(last_action_at)`,
        [userId, actionType, currentOperationId, now],
      );
      return {
        allowed: true,
        retryAfterSeconds: 0,
  ...
  async function isOperationFileProcessed(userId, actionType, operationId, fileKey) {
    ...
    const [rows] = await mySqlService.query(
      `SELECT id FROM cloud_sync_operation_files
       WHERE user_id = ? AND action_type = ? AND operation_id = ? AND file_key = ?
       LIMIT 1`,
      [userId, actionType, operationId, fileKey],
    );
  ...
  async function markOperationFileProcessed(userId, actionType, operationId, fileKey) {
    ...
    await mySqlService.query(
      `INSERT IGNORE INTO cloud_sync_operation_files (user_id, action_type, operation_id, file_key, created_at)
       VALUES (?, ?, ?, ?, ?)`,
      [userId, actionType, operationId, fileKey, new Date()],
    );
  }
  ...
  async function handleListFiles(req, res) {
    ...
    if (!auth) return true;
    const [fileRefs] = await mySqlService.query(
      `SELECT * FROM user_file_refs
       WHERE user_id = ? AND file_key IN (?, ?)`,
      [auth.user.id, "config.json", "play_sessions.db"],
    );
    sendJson(res, 200, {
      files: Object.keys(SUPPORTED_FILES).map((fileKey) => serializeFileRef(fileRefs.find((item) => item.file_key === fileKey))),
    });
    return true;
  }
  ...
  async function handleUpload(req, res, spec) {
    ...
    if (expectedVersion !== null && expectedVersion !== (currentFile?.version || 0)) {
      sendJson(res, 409, {
        ...
        currentVersion: currentFile?.version || 0,
      });
      return true;
    }
    ...
    const uploadStream = mongoService.getBucket().openUploadStream(`${String(auth.user.id)}/${spec.fileKey}`, {
      contentType: String(req.headers["content-type"] || spec.contentType),
      metadata: {
        userId: auth.user.id,
    ...
    try {
      ...
      await mySqlService.query(
        `INSERT INTO user_file_refs (user_id, file_key, file_storage_id, version, size, sha256, content_type, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        [
          nextRef.user_id,
          nextRef.file_key,
          nextRef.file_storage_id,
          nextRef.version,
          nextRef.size,
          nextRef.sha256,
          nextRef.content_type,
          nextRef.created_at,
          nextRef.updated_at,
        ],
      );
