...
const POST_IDS = Array.from({ length: 11 }, (_, index) =>
...
test("SQL null timestamps remain null instead of becoming the Unix epoch", () => {
  ...
  assert.equal(isoTime(new Date("2026-08-23T00:00:00.000Z")), "2026-08-23T00:00:00.000Z");
});
...
function createService({ searchEnabled = false } = {}) {
  ...
  const mySqlService = {
    ...
    transaction: async (callback) => callback({
      query: async (sql) => {
        if (sql.includes("SELECT id FROM forum_posts")) return [[{ id: POST_IDS[0] }]];
        if (sql.startsWith("INSERT IGNORE INTO forum_post_likes")) return [{ affectedRows: 1 }];
        if (sql.startsWith("UPDATE forum_posts")) return [{ affectedRows: 1 }];
        if (sql.includes("AS like_count FROM forum_posts")) return [[{ like_count: 1 }]];
        return [[]];
      },
    }),
  };
...
function createDeleteService({ userId = 7, post, comment, admin = false } = {}) {
  const calls = [];
  const config = {
    RELAY_TOKEN: "relay-token",
    FORUM_SEARCH_WORKER_INTERVAL_MS: 5000,
  ...
  const mySqlService = {
    ...
    transaction: async (callback) => callback({
      query: async (sql, params) => {
        ...
        if (sql.includes("SELECT id, author_user_id, status FROM forum_posts")) return [[post].filter(Boolean)];
        if (sql.includes("SELECT id, status FROM forum_posts")) return [[post].filter(Boolean)];
        if (sql.includes("SELECT post_id FROM forum_comments")) return [[{ post_id: comment?.post_id || POST_IDS[0] }].filter(() => Boolean(comment))];
        if (sql.includes("SELECT id, post_id, author_user_id, status FROM forum_comments")) return [[comment].filter(Boolean)];
  ...
  const service = createForumService({
    ...
    authService: {
      getClientSessionFromRequest: async () => ({
        status: "authenticated",
        auth: { user: { id: userId, github_id: String(userId), role: "player" } },
      }),
      sendAuthFailure: () => {},
    },
    accessControlService: { requireCapability: async () => admin ? { user: { id: 99 } } : null },
    rateLimitService: {},
    sensitiveWordService: { filterText: (value) => value },
...
test("forum feed uses a stable ten-item cursor page", async () => {
  ...
  await service.handleRequest(
    { method: "GET", url: "/api/v1/forum/posts?limit=10", headers: { "x-relay-token": "relay-token" } },
    result,
...
test("forum post details include the author's nickname", async () => {
  const service = createService();
  const result = response();
  await service.handleRequest(
    { method: "GET", url: `/api/v1/forum/posts/${POST_IDS[0]}`, headers: { "x-relay-token": "relay-token" } },
    result,
...
test("forum search is explicitly unavailable when Elasticsearch is not configured", async () => {
  const result = response();
  await createService().handleRequest(
    { method: "GET", url: "/api/v1/forum/posts?limit=10&q=方块", headers: { "x-relay-token": "relay-token" } },
    result,
    new URL("http://localhost/api/v1/forum/posts?limit=10&q=%E6%96%B9%E5%9D%97"),
  );
...
test("forum search status is authenticated and disabled without Elasticsearch", async () => {
  const result = response();
  await createService().handleRequest(
    { method: "GET", url: "/api/v1/forum/search-status", headers: { "x-relay-token": "relay-token" } },
    result,
...
test("forum like endpoints return the transactionally updated count", async () => {
  const result = response();
  await createService().handleRequest(
    { method: "PUT", url: `/api/v1/forum/posts/${POST_IDS[0]}/like`, headers: { "x-relay-token": "relay-token" } },
    result,
    new URL(`http://localhost/api/v1/forum/posts/${POST_IDS[0]}/like`),
  );
  assert.equal(result.status, 200);
  assert.deepEqual(JSON.parse(result.body), { ok: true, liked: true, likeCount: 1 });
});

test("forum resource IDs fail with a client error instead of an internal error", async () => {
  const result = response();
  await createService().handleRequest(
    { method: "GET", url: "/api/v1/forum/posts/not-a-uuid", headers: { "x-relay-token": "relay-token" } },
    result,
...
test("forum authors can delete their own post and cascade active comments", async () => {
  ...
  const result = response();
  await service.handleRequest(
    { method: "DELETE", url: `/api/v1/forum/posts/${POST_IDS[0]}`, headers: { "x-relay-token": "relay-token" } },
    result,
    new URL(`http://localhost/api/v1/forum/posts/${POST_IDS[0]}`),
  );
  assert.equal(result.status, 200);
  assert.deepEqual(JSON.parse(result.body), { ok: true });
  assert.ok(calls.some(({ sql }) => sql.includes("SET status = 1") && sql.includes("forum_posts")));
  assert.ok(calls.some(({ sql }) => sql.includes("SET status = 1") && sql.includes("forum_comments")));
  assert.ok(calls.some(({ sql }) => sql.includes("comment_count = 0") && sql.includes("forum_posts")));
});
...
test("forum authors cannot delete another user's post", async () => {
  ...
  assert.equal(calls.some(({ sql }) => sql.includes("UPDATE forum_posts")), false);
});

test("forum authors can delete their own comment and decrement the parent count", async () => {
  const { service, calls } = createDeleteService({
    post: { id: POST_IDS[0], status: 0 },
    comment: { id: POST_IDS[1], post_id: POST_IDS[0], author_user_id: 7, status: 0 },
  });
  const result = response();
  await service.handleRequest(
    { method: "DELETE", url: `/api/v1/forum/comments/${POST_IDS[1]}`, headers: { "x-relay-token": "relay-token" } },
    result,
  ...
  assert.deepEqual(JSON.parse(result.body), { ok: true });
  assert.ok(calls.some(({ sql }) => sql.includes("SET status = 1") && sql.includes("forum_comments")));
  assert.ok(calls.some(({ sql }) => sql.includes("UPDATE forum_posts SET comment_count")));
});
...
test("forum administrators use status 2 and preserve deleted_at audit time", async () => {
  ...
  assert.equal(result.status, 200);
  assert.deepEqual(JSON.parse(result.body), { ok: true });
  assert.ok(calls.some(({ sql }) => sql.includes("SET status = 2") && sql.includes("deleted_at")));
  assert.ok(calls.some(({ sql }) => sql.includes("comment_count = 0") && sql.includes("forum_posts")));
});

test("only an authorized portal session can restore a post", async () => {
  const { service } = createRestoreService({ authorized: false, post: { id: POST_IDS[0], status: 2 } });
  const result = response();
  await service.handleRequest(
    { method: "POST", url: `/api/admin/v1/forum/posts/${POST_IDS[0]}/restore`, headers: { origin: "https://games.example" } },
    result,
    new URL(`https://games.example/api/admin/v1/forum/posts/${POST_IDS[0]}/restore`),
  );
...
test("restoring a post clears its deletion state without restoring comments and queues search upsert", async () => {
  const { service, calls } = createRestoreService({ post: { id: POST_IDS[0], status: 2 } });
  const result = response();
  await service.handleRequest(
    { method: "POST", url: `/api/admin/v1/forum/posts/${POST_IDS[0]}/restore`, headers: { origin: "https://games.example" } },
    result,
  ...
  assert.equal(result.status, 200);
  assert.deepEqual(JSON.parse(result.body), { ok: true });
  assert.ok(calls.some(({ sql }) => sql.includes("SET status = 0") && sql.includes("forum_posts") && sql.includes("deleted_at = NULL") && sql.includes("deleted_by = NULL")));
  assert.equal(calls.some(({ sql }) => sql.includes("forum_comments")), false);
  assert.ok(calls.some(({ sql, params }) => sql.includes("INSERT INTO forum_search_outbox") && params[1] === "upsert"));
});

test("restoring an active post is idempotent", async () => {
  const { service, calls } = createRestoreService({ post: { id: POST_IDS[0], status: 0 } });
  const result = response();
  await service.handleRequest(
    { method: "POST", url: `/api/admin/v1/forum/posts/${POST_IDS[0]}/restore`, headers: { origin: "https://games.example" } },
    result,
  ...
  assert.equal(result.status, 200);
  assert.equal(calls.some(({ sql }) => sql.startsWith("UPDATE forum_posts")), false);
  assert.equal(calls.some(({ sql }) => sql.includes("forum_search_outbox")), false);
});
...
test("restoring a comment makes it visible and increments the parent count", async () => {
  ...
  const result = response();
  await service.handleRequest(
    { method: "POST", url: `/api/admin/v1/forum/comments/${POST_IDS[1]}/restore`, headers: { origin: "https://games.example" } },
    result,
    new URL(`https://games.example/api/admin/v1/forum/comments/${POST_IDS[1]}/restore`),
  );
  assert.equal(result.status, 200);
  assert.ok(calls.some(({ sql }) => sql.includes("SET status = 0") && sql.includes("forum_comments") && sql.includes("deleted_at = NULL") && sql.includes("deleted_by = NULL")));
  assert.ok(calls.some(({ sql }) => sql.includes("UPDATE forum_posts SET comment_count = comment_count + 1")));
});
...
test("restoring an active comment does not increment the parent count", async () => {
  ...
  assert.equal(result.status, 200);
  assert.equal(calls.some(({ sql }) => sql.startsWith("UPDATE forum_comments")), false);
  assert.equal(calls.some(({ sql }) => sql.includes("comment_count = comment_count + 1")), false);
});
